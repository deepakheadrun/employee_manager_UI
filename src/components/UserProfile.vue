<template>
  <div class="min-h-screen bg-gray-200">
    ...
    <div v-if="activeTab == 1 && userData!=null" class="px-4 py-2 m-2">
      <BasicInfo v-on:updateBasicInfo="updateBasicInfo($event)" v-bind:userdata='this.userData' v-bind:f_name='this.item.first_name' v-bind:l_name='this.item.last_name' v-bind:email='this.email' v-bind:interested_area="this.interested_area" v-bind:isEditable="this.isEditable" />
    </div>
    <div v-if="activeTab == 2" class="px-4 py-2 m-2">
      <Work v-bind:skills='this.skills' v-bind:experience="this.experience " v-bind:isEditable="this.isEditable"  />
    </div>
    <div v-if="activeTab==3" class="px-4 py-2 m-2">
      <Performance v-bind:userdata='this.userData' v-bind:performance='this.performance' v-bind:comments="this.comments" />
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    if (!this.item) {
      ...
      (this.name = this.item.first_name+" "+this.item.last_name), (this.email = this.item.email);
  ...
  let tokens = JSON.parse(localStorage.getItem("tokens"));
      ...
      axios
        .put(process.env.VUE_APP_API_URL + "info/" + this.userData.id + "/",bodyFormData,{
        ...
        .then(
          (response) => {
            this.getImageUrl(response.data.image)
          }
        );
    },
    onTabsClick(index) {
    ...
    getImageUrl(url){
     ...
     if (url && url != "") {
        this.imageUrl = "http://127.0.0.1:8000" +url;
      } else {
        this.imageUrl =
    ...
    getUserInfo(id) {
      ...
      axios
        .get(process.env.VUE_APP_API_URL + "info/" + id + "/", {
          headers: {
            Authorization: "Bearer " + tokens.access,
          },
        })
      ...
      axios
        ...
        .then(
          (response) => {
           ...
           this.interested_area=[...response.data]
          }
        );
    },
    getSiklls(id){
    ...
    getExperience(id){
      let tokens = JSON.parse(localStorage.getItem("tokens"));
      axios
        .get(process.env.VUE_APP_API_URL + "experience/" + id + "/", {
          headers: {
            Authorization: "Bearer " + tokens.access,
          },
        })
